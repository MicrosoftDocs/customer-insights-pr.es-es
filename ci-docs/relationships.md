...
description: Cree y administre relaciones entre entidades de múltiples orígenes de datos.
ms.date: 09/27/2021
ms.reviewer: mhart
...
3. En el panel **Nueva relación**, proporcione la siguiente información:
   ...
   - **Cardinalidad de fuente**: cardinalidad de la entidad de origen. La cardinalidad describe el número de elementos posibles en un conjunto. Siempre se relaciona con la cardinalidad del objetivo. Puedes elegir entre **Uno** y **Muchos**. Solo se admiten relaciones de varios a uno y de uno a uno.  
   ...
   - **Cardinalidad de destino**: cardinalidad de los registros de la entidad de destino.
   - **Campo de clave de origen**: campo de clave externa en la entidad de origen. Ejemplo: SupportCase usa **CaseID** como el campo de clave externa.
   - **Campo clave de destino**: campo clave de la entidad de destino. Ejemplo: el cliente usa **CustomerID** como campo clave.
...
1. En el panel **Jerarquía de cuentas**, proporcione un nombre para la jerarquía. El sistema crea un nombre para la entidad de salida, pero puede cambiarse.
1. Seleccione la entidad que contiene la jerarquía de cuentas. Suele estar en la misma entidad que contiene las cuentas.
1. Seleccione el **UID de cuenta** y el **UID primario** de la entidad seleccionada.
1. Seleccione **Guardar** para finalizar la jerarquía de cuentas.

## <a name="manage-existing-relationships"></a>Administrar las relaciones existentes

Vaya a la página **Relaciones** para ver todas los relaciones que se han creado, su entidad de origen, la entidad de destino y la cardinalidad.
...
Utilice las opciones **Filtrar por** o **Buscar Relaciones** para localizar una relación en particular. Para ver un diagrama de red de las relaciones existentes y su cardinalidad, seleccione [**Visualizador**](#explore-the-relationship-visualizer).
...
Una ruta de relación describe las entidades que están conectadas con Relaciones entre una entidad de origen y una entidad de destino. Se utiliza al crear un segmento o una medida que incluye otras entidades distintas de la entidad de perfil unificado y existen múltiples opciones para llegar a la entidad de perfil unificado. Diferentes rutas de relación pueden producir resultados diferentes.
